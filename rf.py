# # '''
# # Version 5.3 (Re-prediction Loop)
# # - To improve the validation rate, a re-prediction loop has been added.
# # - If a prediction is found to be invalid, the script will try again up to
# #   `MAX_REPREDICTION_TRIES` times.
# # - Each new attempt adds a small amount of noise to the raw Stage 2 output
# #   before post-processing, allowing it to find a valid result.
# # '''
...
# from tensorflow.keras.layers import Input, LSTM, Dense, RepeatVector, TimeDistributed, Attention, Concatenate, GaussianNoise
...
#     TRAINING_FILE = 'train_new.csv'
#     TEST_FILE_1 = 'test_new.csv'
#     TEST_FILE_2 = 'standard_df.csv'
...
# DL_BATCH_SIZE = 32
# NOISE_LEVEL = 0.1
# MAX_REPREDICTION_TRIES = 3 # New: Number of times to try and fix an invalid prediction


# # Output folder setup
# current_time_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
# OUTPUT_FOLDER = f"results_{current_time_str}_v5.3_Repredict"
...
#     lstm_out = LSTM(128, return_sequences=True, name='lstm_layer')(repeated_features)
...
#     print("Stage 2 LSTM Model Summary:")
#     model.summary()
#     return model
...
#     scalers['s2_base_s1'] = scaler_base_s1
#     scalers['s2_ms2'] = scaler_ms2

#     Y_train_s2_reshaped = Y_train[STAGE_2_TARGETS].values.reshape(-1, NUM_SEQ_OUTPUTS, 1)

#     print("  4. Training Stage 2 Sequential (LSTM+Attention) model...")
...
#     return models, reducer, feature_columns

# def evaluate_pipeline(df_test_orig, models, reducer, feature_columns):
#     df_test_filtered = df_test_orig.copy().reset_index(drop=True)
#     if df_test_filtered.empty: return 0.0, 0.0, pd.DataFrame()

#     X_test_base = preprocess_base_features(df_test_filtered, is_train_phase=False)
#     X_test_base = X_test_base.reindex(columns=feature_columns['s1'], fill_value=0)
#     Y_test = df_test_filtered[ALL_TARGETS]
#     pred_test_s1 = np.round(models['s1'].predict(X_test_base)).astype(int)

#     X_test_ms2_raw = process_ms2_spectra(df_test_filtered)
...
#     X_test_s2_base_s1_scaled = scalers['s2_base_s1'].transform(X_test_s2_base_s1)
#     X_test_s2_ms2_scaled = scalers['s2_ms2'].transform(X_test_s2_ms2)
...
#             num_chain_ref = df_test_filtered.iloc[i]['num_chain']
...
#         if final_pred_for_sample is None:
#             final_pred_for_sample = final_pred
#             is_valid_for_sample = False
...
#     Y_test_s2 = Y_test[STAGE_2_TARGETS]
#     true_canonical = [canonicalize_chains(row).tobytes() for _, row in Y_test_s2.iterrows()]
#     pred_canonical = [canonicalize_chains(row).tobytes() for row in predictions_final]
    
#     accuracy = accuracy_score(true_canonical, pred_canonical)
#     validation_rate = np.mean(validity_checks) if validity_checks else 0.0
    
#     df_pred_out = df_test_filtered.copy()
#     pred_cols = [f'pred_{t}' for t in STAGE_2_TARGETS]
#     df_pred_out[pred_cols] = predictions_final
#     df_pred_out['is_valid_prediction'] = validity_checks
...
# # --- 4. Run Pipeline ---
# results = {}
# condition_name = "Trained_Complete_Set"
# df_train_current = df_train.copy().reset_index(drop=True)

# print(f"\n--- Running Pipeline for Training Condition: {condition_name} ---")
# print(f"Training set size: {len(df_train_current)}")

# X_train_base = preprocess_base_features(df_train_current, is_train_phase=True)
# Y_train = df_train_current[ALL_TARGETS]

# models, reducer, feature_cols = train_pipeline(X_train_base, Y_train, df_train_current)
...
# print("Models, reducer, and scalers saved.")


# # --- 5. Evaluate and Save Predictions ---
# print(f"\n--- Evaluating for Condition: {condition_name} ---")
# for dataset_name, df_test in all_test_dfs.items():
#     print(f"  on: {dataset_name} (Size: {len(df_test)})")
#     accuracy, validation_rate, df_predictions = evaluate_pipeline(df_test, models, reducer, feature_cols)
    
#     if dataset_name not in results: results[dataset_name] = {}
#     results[dataset_name]['Accuracy'] = accuracy
#     results[dataset_name]['Validation_Rate'] = validation_rate
...
# '''
# Version 5.7 (Hybrid Single-Chain and ML Approach)
# - Integrated a deterministic, formula-based calculation for all lipids
#   where `num_chain == 1`.
# - These single-chain lipids are now excluded from ML model training.
# - During evaluation, single-chain lipids are processed via direct calculation,
#   while multi-chain lipids are processed by the trained ML models.
# - The final results are a combination of both methods, leveraging the
#   accuracy of direct calculation for simpler cases.
# '''
...
import tensorflow as tf
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input, LSTM, Dense, RepeatVector, TimeDistributed, Attention, Concatenate, GaussianNoise, Conv1D
from tensorflow.keras.callbacks import EarlyStopping
...
# Output folder setup
current_time_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
OUTPUT_FOLDER = f"results_{current_time_str}_v5.7_BestModel"
if not os.path.exists(OUTPUT_FOLDER):
    os.makedirs(OUTPUT_FOLDER)
...
def process_single_chain_lipids(df_single_chain):
    ...
    for _, row in df_single_chain.iterrows():
        ...
        # FIX: Ensure formula is a string to prevent TypeError with re.findall
        ...
        predictions.append(pred_s2_array.tolist())

    results_df = df_single_chain.copy()
    results_df['num_c'] = df_single_chain['num_c']
    results_df['num_db'] = df_single_chain['num_db']
    results_df['predicted_chains'] = predictions
...
def build_stage2_lstm_model(num_base_s1_features, num_ms2_features):
    ...
    concatenated_features = Concatenate(name='concatenate_features')([input_base_s1, noisy_ms2])
    repeated_features = RepeatVector(NUM_SEQ_OUTPUTS, name='repeat_vector')(concatenated_features)
    lstm_out = LSTM(64, return_sequences=True, name='lstm_layer')(repeated_features) # Simplified
    attention_out = Attention(name='attention_layer')([lstm_out, lstm_out])
    output = TimeDistributed(Dense(1, activation='relu'), name='output_layer')(attention_out)
    model = Model(inputs=[input_base_s1, input_ms2], outputs=output)
    model.compile(optimizer='adam', loss='mean_squared_error')
...
def build_stage2_mlp_model(num_base_s1_features, num_ms2_features):
    ...
    noisy_ms2 = GaussianNoise(NOISE_LEVEL, name='gaussian_noise')(input_ms2)
    concatenated_features = Concatenate(name='concatenate_features')([input_base_s1, noisy_ms2])
    dense1 = Dense(128, activation='relu')(concatenated_features) # Simplified
    repeated_features = RepeatVector(NUM_SEQ_OUTPUTS, name='repeat_vector')(dense1)
...
def build_stage2_cnn_model(num_base_s1_features, num_ms2_features):
    ...
    concatenated_features = Concatenate(name='concatenate_features')([input_base_s1, noisy_ms2])
    repeated_features = RepeatVector(NUM_SEQ_OUTPUTS, name='repeat_vector')(concatenated_features)
    conv1 = Conv1D(filters=32, kernel_size=3, activation='relu', padding='same')(repeated_features) # Simplified
    attention_out = Attention(name='attention_layer')([conv1, conv1])
    output = TimeDistributed(Dense(1, activation='relu'), name='output_layer')(attention_out)
    model = Model(inputs=[input_base_s1, input_ms2], outputs=output)
    model.compile(optimizer='adam', loss='mean_squared_error')
    return model
...
def evaluate_pipeline(df_test_orig, models, reducer, feature_columns, current_scalers):
    ...
    if not df_multi_chain.empty:
        ...
        for i in range(len(pred_test_s2_raw)):
            ...
            if not is_valid_for_sample:
                num_chain_ref = df_multi_chain.iloc[i]['num_chain']
                num_c_ref, num_db_ref = pred_test_s1[i, 0], pred_test_s1[i, 1]
                chains = last_attempted_pred.reshape(-1, 2)
                
                if num_chain_ref > 0:
                    c1, db1 = chains[0]
                    rem_c = num_c_ref - c1
                    rem_db = num_db_ref - db1
                    
                    new_chains = np.zeros_like(chains)
                    new_chains[0] = [c1, db1]
                    
                    rem_chain_count = num_chain_ref - 1
                    if rem_chain_count > 0:
                        c_per_chain = rem_c // rem_chain_count
                        db_per_chain = rem_db // rem_chain_count
                        for k in range(1, num_chain_ref): new_chains[k] = [c_per_chain, db_per_chain]
                        c_rem = rem_c % rem_chain_count
                        db_rem = rem_db % rem_chain_count
                        for k in range(c_rem): new_chains[1+k, 0] += 1
                        for k in range(db_rem): new_chains[1+k, 1] += 1
                    final_pred_for_sample = new_chains.flatten()
                else:
                    final_pred_for_sample = np.zeros_like(last_attempted_pred)
                is_valid_for_sample = False
        ...
        multi_chain_results_df['num_c'] = df_multi_chain['num_c']
        multi_chain_results_df['num_db'] = df_multi_chain['num_db']
        multi_chain_results_df['predicted_chains'] = [canonicalize_chains(p).tolist() for p in predictions_final]
    ...
    Y_test = df_test_orig.loc[df_pred_out.index][ALL_TARGETS]
    true_canonical = [canonicalize_chains(row).tolist() for _, row in Y_test[STAGE_2_TARGETS].iterrows()]
    
    accuracy = accuracy_score([str(x) for x in true_canonical], [str(x) for x in df_pred_out['predicted_chains']])
...
for dataset_name, df_pred in best_predictions.items():
    if not df_pred.empty:
        ...
        output_cols = ['num_c', 'num_db', 'predicted_chains', 'is_valid_prediction']
        df_to_save = df_pred[output_cols]
        pred_filename = os.path.join(OUTPUT_FOLDER, f"predictions_{dataset_name}.csv")
        df_to_save.to_csv(pred_filename, index=False)
        print(f"Predictions for {dataset_name} saved.")
